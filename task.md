# TASK — Rewrite PortalBrandingTab logo upload completely

The label-wrapping approach is still not working. Rewrite the entire component with a completely different upload strategy: use a visible <input type="file"> that is absolutely positioned and covers the drop zone, with opacity 0 and full width/height. This is the most bulletproof cross-browser approach and cannot be intercepted.
...
Replace the ENTIRE existing file with the above. Do not merge or patch — full replacement.