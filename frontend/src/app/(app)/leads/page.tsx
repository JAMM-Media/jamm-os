      ...
      <div className="flex items-center gap-3 mb-5 flex-wrap">
        <div className="relative">
          ...
          <input
            ...
            className="h-8 pl-8 pr-8 rounded-[6px] border border-[0.5px] border-surface-border dark:border-dark-border bg-white dark:bg-dark-card text-[12px] text-brand dark:text-[#EDEEF0] placeholder:text-[#9CA3AF] focus:outline-none focus:border-brand dark:focus:border-[#4A7FA5] w-52 shadow-sm"
          />
          {search && (
            <button
              onClick={() => setSearch('')}
          ...
          <table className="w-full border-collapse">
            <thead>
              <tr className="bg-[#F8FAFC] dark:bg-[#1E2530] border-b border-[0.5px] border-[#E2E8EF] dark:border-dark-border">
                {['Name', 'Email', 'Stage', 'Source', 'Added'].map((col) => (
                  <th key={col} className="px-5 py-3 text-left text-[11px] font-semibold text-[#6B7280] uppercase tracking-[0.07em] whitespace-nowrap">
                    {col}
                  </th>
                ))}
              </tr>
            ...
            <tbody>
              {filtered.map((lead, i) => (
                <tr
                  ...
                  className={cn(
                    'hover:bg-[#F1F5F9] dark:hover:bg-[#2D3748] transition-colors cursor-pointer',
                ...
                >
                  ...
                  <td className={cn(
                    ...
                    <div className="flex items-center gap-2">
                      ...
                      <Link
                        href={`/leads/${lead.id}`}
                        className="text-[13px] font-semibold text-brand dark:text-[#EDEEF0] hover:text-brand-light dark:hover:text-[#4A7FA5] hover:underline"
                      >
                        {lead.name}
                      </Link>
                    </div>
