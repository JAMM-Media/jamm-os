// frontend/src/components/engagements/EngagementEmptyState.tsx

import { Briefcase } from 'lucide-react'

interface EngagementEmptyStateProps {
  onNew: () => void
}

export function EngagementEmptyState({ onNew }: EngagementEmptyStateProps) {
  return (
    <div className="flex flex-col items-center justify-center flex-1 py-24 gap-[10px]">
      <div className="flex items-center justify-center w-10 h-10 rounded-lg bg-surface-card dark:bg-dark-card border border-[0.5px] border-surface-border dark:border-dark-border">
        <Briefcase className="h-[18px] w-[18px] text-[#6B7280]" />
      </div>
      <div className="text-center">
        ...
        <p className="text-[12px] text-[#6B7280]">
          Create an engagement to start tracking client work.
        </p>
      </div>
      <button
        onClick={onNew}
