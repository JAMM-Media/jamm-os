...
export default function PortalBrandingTab() {
  ...
  useEffect(() => {
    api.get('/users/firm').then((res) => {
      ...
      if (settings.portal_logo_s3_key) {
        setLogoPreviewUrl(`${process.env.NEXT_PUBLIC_API_URL ?? 'https://api.jammpx.com'}/firms/logo/${data.id}`)
      }
  ...
  async function processFile(file: File) {
    ...
    try {
      ...
      setLogoPreviewUrl(`${process.env.NEXT_PUBLIC_API_URL ?? 'https://api.jammpx.com'}/firms/logo/${firmId}?t=${Date.now()}`)
