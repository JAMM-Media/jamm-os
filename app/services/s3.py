...
def copy_object_within_bucket(source_key: str, dest_key: str) -> None:
    """
    Server-side copy of an S3 object within the same bucket.
    No bytes are downloaded to the server. Correct for objects up to 5GB
    (this codebase caps uploads at 250MB, so single-part copy always suffices).
    """
    settings = get_settings()
