...
8. **A restore command reporting success after silently failing.** During a negative control, the command that was supposed to restore a mutated file failed on a bad path and printed "restored" anyway. The mutation was still in the file. It was caught only by grepping for the mutation marker instead of trusting the message. This one is a different flavor from the others: not a tool measuring the wrong thing, but a command reporting on an outcome it never checked. Verification steps are themselves subject to this rule. Confirm a restore by inspecting the file, not by reading the message.
    ...
    **Addendum (Aug 22, 2026. Origin: the pricing `delete_config` session, negative control 4's revert).** The same failure arrived by a new route: not a bad path this time, but an ambiguous one. The restore script's anchor text matched TWICE in the target file, because `change_dimension_direction` shares the same query shape as the function under control, and the edit was written against the file as a whole rather than against a region. Ambiguity does not fail uniformly. One script errored loudly and left the mutation sitting in the file, which is the survivable outcome, because it is visible. An earlier control's script silently never applied at all, and that one reported "1 passed" over a mutation that had never been made. The control proved nothing, and it said so in the voice of success. A test that passes because the break never landed is indistinguishable from a test that passes because the guard works.

    The lesson is this instance's original lesson, re-confirmed by a second route: a restore claim proves nothing, and the only way to know what is in a file is to read it. What is new is the tactic. Edit scripts that patch or restore code scope their edit to the target region, splitting at the function definition rather than operating on the whole file, and assert their anchor text is unique before applying. An anchor that matches twice makes the edit ambiguous, and an ambiguous edit can hit the wrong site or silently no-op. Verification is reading the changed block back plus a green re-run, never the script's own exit code.
...
19. Instance nineteen (Aug 26, 2026. Origin: the `app/main.py` scheduler comment removal in the platform_events session). **A file write that changed lines the edit never touched.** `app/main.py` holds 386 CRLF line endings and 2 bare-LF ones, and writing the file back in text mode silently normalized those 2 lines, producing `git diff` hunks on two unrelated `documents_router` imports while the intended five-line deletion was itself correct. The edit script asserted its anchor was unique, applied cleanly, and reported success; nothing but `git diff` revealed the damage.
    ...
    **Addendum (Sep 1, 2026. Origin: appending instance twenty to this file during the surface_items build).** The numstat comparison above catches damage to lines the edit never touched. It does NOT catch wrong endings on lines the edit ADDS: an added line counts as added under both `--numstat` and `--ignore-cr-at-eol --numstat`, so writing CRLF lines into an LF file passes the check clean. The gap matters here because the repo is genuinely mixed: this file is pure LF while every Python file beside it is pure CRLF, `core.autocrlf` is true, and there is still no `.gitattributes`. An append that assumed CRLF, as the first attempt did, would have put the first mixed endings into this file and the prescribed check would have blessed it.

    The arbiter is a byte count, not a diff: count CRLF pairs against bare LF bytes in the file before and after the write, and confirm the result matches the convention the file already had rather than the one you assumed. Read the ending off the file, then write that.
